...
Find flaws in _what the app allows_, not _how it's coded_. These are valid requests that
...
**Core principle:** The app enforces the _intended_ flow; you test every _unintended_ path that
...
- **Skip when:** it's a stateless single-action utility with no rules or state → N/A (rare —
  most apps have _some_ logic; double-check before skipping).
...
- Looking for code smells — logic bugs are in _valid_ requests; you must know the _rules_.

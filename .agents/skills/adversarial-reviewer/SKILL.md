...
### 8. AFA Portal & UAE Guide Compliance

- **Header Tunisien :** Drapeau, mention « République Tunisienne », nom du ministère de tutelle, logo présents obligatoirement sur toutes les pages.
- **Utilisabilité & Navigation :** Fil d'Ariane cliquable, menu actif constant, lien "Accueil" partout, liens visités de couleur différente, recherche en haut avec bouton "Chercher".
- **Accessibilité (WCAG 2.1 AA) :** Attributs `alt` sur images, navigation 100% au clavier (Tab) avec focus visuel, avertissement pour les liens ouvrant de nouvelles fenêtres (`target="_blank"`), pas de redirection ou rafraîchissement automatique côté client.
- **Sécurité & Rôles ([auth-plan.md](../afa-portal-development/references/auth-plan.md)) :** Rate limiting sur login/password, pas de self-signup sur l'admin `/admin`, mapping correct des rôles (ADMIN/EDITOR/CONTRIBUTOR/SUBSCRIBER), hashing PBKDF2/Argon2id.
- **SQL & Données spatiales (PostgreSQL / PostGIS) :** Requêtes géospatiales paramétrées via Kysely (pas d'interpolation de variables dans `sql`), CRS EPSG:4326/WGS84, validation des géométries de lots.
- **RGPD / Vie Privée :** Mentions légales, cookie policy, cases de consentement explicites non pré-cochées sur formulaires newsletter/réclamations.
- **Performance :** Temps de chargement optimal, requêtes redondantes éliminées via `requestCached`.
- **Migration & Redirections :** Redirections 301 configurées pour toutes les anciennes URLs de `www.afa.nat.tn`.

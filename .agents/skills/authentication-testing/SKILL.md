...
**Core principle:** Auth is a _flow_, not a page. Bugs live in reset, refresh, federation, and
...
- Decoding a JWT and assuming it's verified — _prove_ the signature is checked (tamper it).
- Treating MFA as unbreakable — most bypasses are logic flaws in the _flow_, not the crypto.

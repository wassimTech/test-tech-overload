...
Verify that every object and every action enforces "is _this_ caller allowed?" server-side.
...
**Core principle:** Authentication proves _who you are_; authorization proves _what you may do_.
...
| Horizontal privesc | Act on another _same-level_ user's resources.                        |
...
- Testing only as admin — you'll never see the bug. Always test as the _lower_ privilege.

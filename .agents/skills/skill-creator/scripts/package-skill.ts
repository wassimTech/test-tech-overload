...
async function packageSkill(skillPath: string, outputDir?: string): Promise<string | null> {
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      throw new Error(`zip command failed: ${Buffer.from(result.stderr).toString()}`)
    }
...
if (import.meta.main) {
  ...
  if (args.length < 1) {
    console.log("Usage: bun scripts/package-skill.ts <path/to/skill-folder> [output-directory]")
    console.log("\nExamples:")

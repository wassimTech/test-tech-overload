...
template engine, an XML/LDAP/XPath parser. This skill _proves_ it dynamically; the code-level
patterns live in `security-code-audit/references/database.md` and `api-backend.md`.

**Core principle:** Find where input changes the _structure_ of a command, not just its data.
...
4. **Bound impact** — note what an attacker could reach; do _not_ exfiltrate real data.
...
Per finding: the sink, the structural payload, the _evidence_ of interpretation (error/time/OOB),

...
**Core principle:** Attackers think in _paths_, defenders think in _lists_. A defender must
...
_which_ skill to open at each phase — you MUST actually open it with the `Skill` tool.
...
| FIND/EXPLOIT | _deep-dive specialists (below)_ | Find and validate vulnerabilities.                   |
...
Don't run every skill — run what the target _has_. From `recon-and-osint` + `threat-modeling`,
...
**Always emit a coverage report** so it's clear what ran, what was skipped _and why_, and what
has no coverage:
